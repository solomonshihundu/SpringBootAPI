...
public class WebuserController {
	...
	public ResponseEntity<?> createWebuser(@RequestBody Webuser webuser) {
		try {
			...
			Optional<Webuser> existingWebUser = webuserRepository
					.findByUsernameOrEmailOrEmployeeIdOrCustomerId(username,email,employeeId,customerId);
			if(existingWebUser.isPresent()){
				return ResponseEntity.badRequest().body("Webuser already exists");
			}
